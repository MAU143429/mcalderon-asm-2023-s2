  ...
  if (haveData)
     {
      for(int i = 2; i < SIZE; i++){
          ...
          nums[i] = nums[i] * sinecarrier[i];

          lowpassFilteredSignal2[i] = 0.1 * nums[i] + 0.9 * lowpassFilteredSignal2[i-1];
          //lowpassFilteredSignal2[i] = 0.0001 * nums[i] + 0.0005 * lowpassFilteredSignal2[i-1];


           Serial.println (lowpassFilteredSignal2[i]);

           analogWrite(speakerPin, lowpassFilteredSignal2[i]);
